...
mod coinbase_tests {
    use cex_exchanges::{
        exchanges::coinbase::CoinbaseWsBuilder, types::coinbase::channels::CoinbaseChannel,
    };
    ...
    async fn test_rfq_matches() {
        let builder =
            CoinbaseWsBuilder::default().add_channel(CoinbaseChannel::new_rfq_matches_from_raw(
                vec!["ETH_USD".to_string(), "btc_usd".to_string()],
                '_',
            ));
